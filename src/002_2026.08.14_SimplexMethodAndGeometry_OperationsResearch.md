...
slack: The best answer to a linear problem always sits at a corner, and Dantzig's simplex method just walks between corners until no neighbour is better. This traces why that works geometrically, what a pivot actually does to a tableau, and then the awkward part: Klee and Minty built a cube where simplex visits every vertex exponentially, yet in seventy years of industrial use it has essentially never happened. An algorithm with a catastrophic worst case that nobody has ever hit is a strange thing to trust, and worth understanding why we do.
burned:
